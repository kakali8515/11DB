<template>
  <div class="media-from-details">
    <div
      :class="[title_type == 'tv' ? 'page-heading-area  with-dropdown edit-drop-outer' : 'page-heading-area', $route.params.type == 'internal' ? 'tmtb-heading' : 'tmtb-heading']">
      <div class="heading-dropdown tmtb-fill-outer">
        <h2>{{ $t("addNewMoviesForm.media") }}</h2>
        <template v-if="title_type == 'tv'">
          <SearchDropdown @dropdownValue="getSeasonId" :search_type="selectSeason" :searchTitleListItem="seasonData"
            :searchtitle="seasonData.length > 0 ? (item) => `${$t('credit.season')} ${item.season_no}` : ''" return-object
            :clearable="false" />
        </template>
      </div>
      <div class="media-tmtb-outer">
        <div class="form-group number-id" v-if="$route.params.type == 'internal'">
          <div class="tmtb-fill">
            <input type="text" v-model="tmdb_id" class="form-control" :placeholder="$t('addNewMoviesForm.TMDBID')"
              @keypress="isIntegerNumber($event)" readonly id="" name="" />
        ...
        <template v-if="tabName == 'addNewPeople.mediadetails.videoTitle'">
          <div class="media-save-btn" v-if="title_type == 'movie'" :class="video_details.length &&
            routeId == '' && $route.params.type != 'internal' ? 'isDisabled' : ''">
            <a v-if="!isEdit" @click="AddMediaDetails" :class="video_details.length &&
              routeId == '' && $route.params.type != 'internal' ? 'pointer-events-none' : ''" class="saveBtn">{{
    $t("button.save") }}</a>
            <a v-else @click="isEditBtn" class="saveBtn">{{
              $t("button.edit")
            }}</a>
          </div>
          <div class="media-save-btn" v-if="title_type == 'tv'"
        ...
        <template v-if="tabName == 'mediaSection.images'">
          <div class="media-save-btn" v-if="title_type == 'movie'" :class="(image_list.length == 0 && !background_image_url) ||
            (routeId == '' && $route.params.type != 'internal') ? 'isDisabled' : ''">
            <a v-if="!isEdit" @click="AddMediaDetails" :class="(image_list.length == 0 && !background_image_url) ||
              (routeId == '' && $route.params.type != 'internal') ? 'pointer-events-none' : ''" class="saveBtn">{{
    $t("button.save") }}</a>
            <a v-else @click="isEditBtn" class="saveBtn">{{
              $t("button.edit")
        ...
        <template v-if="tabName == 'addNewMoviesForm.poster'">
          <div class="media-save-btn" v-if="title_type == 'movie'"
            :class="poster_image_list.length == 0 ? 'isDisabled' : ''">
            <a v-if="!isEdit" @click="AddMediaDetails" :class="poster_image_list.length == 0 ? 'pointer-events-none' : ''
              " class="saveBtn">{{ $t("button.save") }}</a>
            <a v-else class="saveBtn" @click="isEditBtn">{{
              $t("button.edit")
            }}</a>
          ...
          <div class="thumb-grid-area">
            <template v-if="video_details.length > 0">
              <template v-for="(video, index) in video_details" :key="index">
                <VideoThumb v-if="video.action_type != 'd'" :video="video" @removeVideoItem="removeVideoItem"
                ...
                <div class="upload-options">
                  <label for="bg-picker-1" :class="(routeId == '' && $route.params.type != 'internal') ||
                    ...
                    <input type="file" :disabled="(routeId == '' && $route.params.type != 'internal') ||
                      isEdit
                      " class="image-upload" accept="image/*" v-on:change="onFileChange" id="bg-picker-1" />
            ...
            <div class="images-grid-area">
              <template v-if="image_details.length > 0">
                <template v-for="(image, index) in image_details" :key="index">
                  <ImageThumb v-if="image.action_type != 'd'" :image="image" @removeItem="removeImagesItem"
                    :isEdit="isEdit" />
                </template>
              </template>
            ...
            <div class="poster-grid-area">
              <template v-if="poster_details.length > 0">
                <template v-for="(poster, index) in poster_details" :key="index">
                  <PosterThumb v-if="poster.action_type != 'd'" :poster="poster" @removeItem="removePosterItem"
                    @setPoster="setMainPoster" :isEdit="isEdit" />
                </template>
              </template>
              <div class="tv-no-data" v-else>
                {{ $t("addNewMoviesForm.noData") }}
  ...
  <div class="new-cast-modal" v-show="isVideoModal">
    ...
    " maxlength="250" />
            ...
            <div class="form-grp">
              ...
              <input type="text" id="" name="" :placeholder="$t('addNewMoviesForm.entervideotitle')" v-model="video_title"
                maxlength="250" />
              <div class="add-check">
...
export default {
  ...
  data() {
    return {
      ...
      multipleImages: [],
      images: [],
      title_type: this.itemType == "tv" ? "tv" : "movie",
      video_details: [],
      image_details: [],
      poster_details: [],
      is_main_poster: "",
  ...
  methods: {
    ...
    tmdbRefreshClick() {
      ...
      if (this.itemType == "tv") {
        this.tmdbClickVideo = this.video_details;
        this.tmdbClickImage = this.image_list;
        this.tmdbClickPoster = this.poster_image_list;
        this.tmdbClickBgImage = this.background_image_object;
        this.tmdbClickVideo.map((i) => {
          i.action_type = "d";
        });
        this.tmdbClickBgImage.map((i) => {
          i.action_type = "d";
        });
        this.tmdbClickImage.map((i) => {
          i.action_type = "d";
        });
        this.tmdbClickPoster.map((i) => {
          i.action_type = "d";
        });
      }
    ...
    getVideoList() {
      ...
      if (this.$route.params.type == "internal") {
        if (this.istmDBIdClick) {
          videoList = this.EditMovieService.editTmdbMediaRequestList(
            this.title_id,
            this.site_language,
            this.title_type,
            "video",
          ...
          videoList = this.EditMovieService.editMediaRequestList(
            this.title_id,
            this.routeId,
            this.site_language,
            this.title_type,
        ...
        videoList = this.AddNewMovieService.mediaRequestList(
          this.routeId,
          this.site_language,
          this.title_type,
          "video",
          ...
          .then((res) => {
            if (res.status == 200) {
              ...
              Array.from(res.data.video_details).forEach((item, i) => {
                ...
                this.video_details[i]["action_type"] = this.istmDBIdClick && this.itemType == "tv" ? "a" : "e";
              });
            }
          })
          .catch((err) => {
    ...
    getImageList() {
      ...
      if (this.$route.params.type == "internal") {
        if (this.istmDBIdClick) {
          imageList = this.EditMovieService.editTmdbMediaRequestList(
            this.title_id,
            this.site_language,
            this.title_type,
            "image",
          ...
          imageList = this.EditMovieService.editMediaRequestList(
            this.title_id,
            this.routeId,
            this.site_language,
            this.title_type,
        ...
        imageList = this.AddNewMovieService.mediaRequestList(
          this.routeId,
          this.site_language,
          this.title_type,
          "image",
          ...
          .then((res) => {
            if (res.status == 200) {
              ...
              this.draft_media_id = res.data.image_details[0]?.draft_media_id
                ...
                : this.draft_media_id;
              this.background_image_url =
                res.data.bg_image_details[0].image_path;

              this.background_image_object.push({
                action_type: this.istmDBIdClick && this.itemType == "tv" ? "a" : "e",
                originalname: res.data.bg_image_details[0].image_original_name,
                filename:
                  res.data.bg_image_details[0].image_file_name ||
                  res.data.bg_image_details[0].image_original_name,
              ...
              Array.from(this.image_details).forEach((item, i) => {
                ...
                this.image_details[i]["action_type"] = this.istmDBIdClick && this.itemType == "tv" ? "a" : "e";
    ...
    getPosterList() {
      ...
      if (this.$route.params.type == "internal") {
        if (this.istmDBIdClick) {
          posterList = this.EditMovieService.editTmdbMediaRequestList(
            this.title_id,
            this.site_language,
            this.title_type,
            "poster",
          ...
          posterList = this.EditMovieService.editMediaRequestList(
            this.title_id,
            this.routeId,
            this.site_language,
            this.title_type,
        ...
        posterList = this.AddNewMovieService.mediaRequestList(
          this.routeId,
          this.site_language,
          this.title_type,
          "poster",
          ...
          .then((res) => {
            if (res.status == 200) {
              ...
              Array.from(res.data.poster_details).forEach((item, i) => {
                ...
                this.poster_details[i]["action_type"] = this.istmDBIdClick && this.itemType == "tv" ? "a" : "e";
    ...
    saveMediaDetails() {
      ...
      let bgImagemergedArray = [...this.background_image_object, ...this.tmdbClickBgImage];

      if (this.$route.params.type == "internal") {
        Array.from(videomergedArray).forEach((item) => {
          video.push({
        ...
        Array.from(imagemergedArray).forEach((item, i) => {
          image.push({
            id: item.id ? item.id : "",
            originalname: item.originalname,
        ...
        Array.from(postermergedArray).forEach((item) => {
          poster.push({
            originalname: item.originalname,
            path: item.path,
            is_main_poster: item.is_main_poster,
      ...
      let credentials = {
        ...
        video_list: video,
        background_image: this.istmDBIdClick && this.itemType == "tv" ? bgImagemergedArray : this.background_image_object,
        image_list: image,
      ...
      if (this.$route.params.type == "internal") {
        saveMediaDetails = this.EditMovieService.editMediaDetails(credentials);
      } else {
        saveMediaDetails = this.AddNewMovieService.addMediaDetails(credentials);
      }
      ...
      saveMediaDetails
        .then((res) => {
          if (res.status == 200) {
            ...
            this.saveIdList.request_id = res.data.data[0].draft_request_id;
