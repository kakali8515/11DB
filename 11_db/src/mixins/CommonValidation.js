...
export function isChar(evt) {
  ...
  if (
    ...
    evt.preventDefault();
    return false;
  } else if (evt.which === 32 && evt.currentTarget.selectionStart == 0) {
    evt.preventDefault();
    return false;
  } else {
    return true;
...
// first space is not allow
export function requiredField(evt) {
  evt = evt ? evt : window.event;
  var charCode = evt.which ? evt.which : evt.keyCode;
  if (evt.which === 32 && evt.currentTarget.selectionStart == 0) {
    evt.preventDefault();
    return false;
  } else {
    return true;
