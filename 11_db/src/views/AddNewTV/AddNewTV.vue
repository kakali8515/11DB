<template>
  <div class="add-new-movie">
    <div class="new-container">
      <h1 class="heading" v-html="$t('header.addSection.addNewTvShows')"></h1>
      ...
      <template v-if="isSearch && isTmdbData == false">
        <div class="movie-list-section">
          <h1 class="data-title">{{ searchResultHeading }}</h1>
                ...
                <label :for="'radio-movie-1' + index">
                  ...
                  <div class="movie-content">
                    <div class="movie-image">
                      <img v-if="item.poster_image.includes('https')" :src="item.poster_image" alt="" />
          ...
          <div class="no-data-block" :style="internal_data.results?.length == 0 ? '' : 'display: none'">
            <p class="no-data-text">{{ $t("dictionary.noData") }}</p>
          </div>
        </div>
        <div class="checkbox-section" v-if="internal_data.results?.length > 0">
          <label for="new-movie-check">
            ...
            <p class="check-text">
              {{ $t("addNewTv.theTvShowIsNotOnTheList") }}
            </p>
          </label>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      tvId: "",
      searchResultHeading: this.$t("addNewMovies.internalDataExist"),
      searchTitleListItem: [],
    };
  },
  created() {
  ...
  methods: {
    ...
    searchMovie() {
      ...
      this.AddNewMovieService.searchMovieTitle(
        ...
        .then((res) => {
          if (res.status == 200) {
            this.search_type == "tiving_id"
              ? (this.searchResultHeading = this.$t(
                "addNewMovies.internalDataTiving"
              ))
              : (this.searchResultHeading = this.$t(
                "addNewMovies.internalDataExist"
              ));
    ...
    internalDataNext() {
      ...
      // console.log(this.isInterRadioButton)
      if (this.isInterRadioButton) {
        this.$router.push("/add-tv-details/" + this.tvId + "/internal");
        localStorage.setItem("tvBackPage", "/add-new-tv");
      } else {
    ...
    tmdbDataNext() {
      if (this.isTmdbRadioButton) {
        this.$router.push("/add-tv-details/" + this.tvId + "/tmdb");
        localStorage.setItem("tvBackPage", "/add-new-tv");
      } else {
        this.$router.push("/add-tv-details/" + 0 + "/new");
        localStorage.setItem("tvBackPage", "/add-new-tv");
      }
    },
    clickTmdbPage(click) {
