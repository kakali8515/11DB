<template>
  <div>
    <div class="filters">
      <h3>{{ $t("searchResults.searchRight.movies") }}</h3>
      <span>
    ...
    <div class="search-no-data" v-if="moviesData.length < 1 && showData == false">

      {{ $t("searchResults.noData") }}
    </div>
    <ul class="search-list search-scroll" v-else>
      <li v-for="(item, ind) in moviesData" :key="ind">
        <a href="javascript:void(0)"
          @click="$router.push({ name: 'MoviesDetails', params: { id: item.id, pageTitle: item.title } }); searchLog(item,`/movies/details/${item.id}`)">
          <div class="img-wrap">
