...
export default {
  ...
  setup() {
    ...
    const getEpisodeDetails = async (season_id) => {
      try {
        let response = await TVShowsDetailsService.getEpisodeDetails(
          tvId,
          season_id
        );
        ...
        // currentSeosonId.value = response.data.season_list[0].season_id;
    ...
    const toEditTVEpisode = () => {
      ...
      localStorage.setItem(
        "tvBackPage",
        "/tvshows/details-episodes/" + tvId + "/" + seasonId.value
      );
    };
    return {
      episodeDetails,
