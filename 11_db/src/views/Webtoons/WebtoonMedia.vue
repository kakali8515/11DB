...
export default {
  ...
  setup() {
    ...
    const mediaVideoDetails = async (_id, _page) => {
      ...
      try {
        let response = await WebtoonDetailsService.getMediaDetails(data);
        if (response.status == 200) {
          isData.value = true;
          videos.value = response.data.webtoons_season_meadia_list;
          Array.from(response.data.webtoons_season_meadia_list).forEach((item, i) => {
    ...
    const mediaImageDetails = async (_id, _page) => {
      ...
      try {
        ...
        if (response.status == 200) {
          ...
          images.value = response.data.webtoons_season_meadia_list;
    ...
    const mediaPosterDetails = async (_id, _page) => {
      ...
      try {
        ...
        if (response.status == 200) {
          ...
          posters.value = response.data.webtoons_season_meadia_list;
    ...
    const toAddMediaVideo = () => {
      ...
      localStorage.setItem("webtoonBackPage", "/webtoons/webtoon-media/" + tvId + "/" + seasonId);
    };
