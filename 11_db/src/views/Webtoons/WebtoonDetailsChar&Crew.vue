              ...
              <Tabs title="adminAward.character">
                <div class="tvimage-otr" v-if="newResult.length > 0">
                  <template v-for="(castList, i) in newResult" :key="i">
                    <CastCrewCard :castList="castList" :redirectLink="castList.id ? `PeopleDetails` : ``" />
                  </template>
                </div>
...
export default {
  ...
  setup() {
    ...
    // Lifecycle hooks
    onMounted(() => {
      getCastCrewDetails(tvId, "character", 1);
      ...
      watch(
        ...
        function (newVal, oldVal) {
          if (newVal && oldVal && newVal != oldVal) {
            ...
            else getCastCrewDetails(tvId, "character", 1);
          }
        }
      );
    ...
    const getCastCrewDetails = async (_id, _type, _page) => {
      ...
      try {
        let response = await WebtoonDetailsService.getCastDetails(data);
        if (response.status == 200) {
          ...
          window.document.title = `${response.data.title} - ${response.data.season_name} - ${t("webtoons.CharacterCrew")} - 11DB`;
    ...
    const toEditMovieCredit = () => {
      ...
      localStorage.setItem("webtoonBackPage", "/webtoons/character&crew/" + tvId + "/" + season_id);
    };
    ...
    const onScroll = () => {
      ...
      if (bottomOfWindow && totalRecords.value > newResult.value.length) {
        ...
        setTimeout(() => {
          ...
          else getCastCrewDetails(tvId, "character", newPage);
        }, 100);
      }
    };
