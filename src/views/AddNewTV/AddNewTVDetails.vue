    ...
    <div class="add-new-movie-form">
      <div class="new-container">
        <div class="page-heads-area">
          <h1 class="heading" v-if="$route.params.type !== 'internal'" v-html="$t('header.addSection.addNewTvShows')">
          </h1>
          <h1 class="heading" v-else v-html="$t('tvShows.editTvShows.editTvShow')"></h1>
...
// import PrimaryDetails from "@/components/AddNewTv/PrimaryDetails.vue";
import PrimaryDetails from "@/components/AddNewTv/PrimaryDetails.vue";
...
import Episode from "../../components/AddNewTv/Episode.vue";
import Season from "../../components/AddNewTv/Season.vue";
...
export default {
  ...
  setup() {
    ...
    watch(
      () => route.fullPath,
      (to, from) => {
        if (to.includes('add-tv-details') == false) {
    ...
    document.title = t("pageTitle.editTv") + ' - 11DB';
    ...
    const submitAllData = () => {
      ...
      if (draft_ids[0]) {
        routeId.value = draft_ids[0]?.primaryDetails["tv"]?.draft_relation_id
          ? draft_ids[0]?.primaryDetails["tv"]?.draft_relation_id
          : "";
      } else {
      ...
      submitData
        .then((res) => {
          if (res.status == 200) {
            // Toast.fire({ title: "Data has been successfully submitted" });
    ...
    const goPageBack = () => {
      ...
      router.push(
        localStorage.getItem("tvBackPage")
          ? localStorage.getItem("tvBackPage")
          : "/add-new-tv"
      );
      localStorage.removeItem("tvBackPage");
    };
