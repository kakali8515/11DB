<template>
    <div class="credit-from-details">
        <div :class="[itemType == 'webtoons'
            ...
            <div class="heading-dropdown tmtb-fill-outer">
                ...
                <template v-if="itemType == 'webtoons'">
                    <SearchDropdown @dropdownValue="getSeasonId" :search_type="selectSeason"
                        :searchTitleListItem="seasonData" :searchtitle="seasonData.length > 0
            ...
            <div class="media-tmtb-outer">
                ...
                <div class="media-save-btn" :class="(saveIdList.season_id != '' && routeId != '') ||
        ...
        <div class="credit-tabber-area">
            <TabWrapper @selectedTab="getTabName">
                <Tabs title="adminAward.character">
                    <div class="credit-cast-area">
                        <button :disabled="isEdit" :class="isEdit ? 'isDisabled' : ''" class="add-season-btn mt-0"
                            @click="addNewCharacter">
                            {{ $t("addNewWebtoonForm.addNewCharacter") }}
                        </button>

                        <div class="credit-list-area">
                            <template v-if="cast_details.length > 0">
                                <draggable :disabled="isEdit" v-model="cast_details" item-key="id" :animation="300"
                                    :forceFallback="true" :scroll-sensitivity="200">
                                    <template #item="{ element }">
                                        <div class="single-credit-list webtoon-credit" v-show="element.action_type != 'd'">
                                            <div class="divrow">
                                                <span class="icons">
                                            ...
                                            <div class="inner-pt-area">
                                                <div class="divrow name-wrap">
                                                    <h3>{{ element.character_name }}</h3>
                                                </div>
                                                <div class="divrow center credit-description">
                                                    <p :title="element.description">{{ element.description }}</p>
                                                </div>
                                            </div>
                                            <div class="end-part">
                                                <div class="divrow center">
                                                    <span class="gustBtn" v-if="element.is_guest == '1'">{{
                                                        $t("credit.guest") }}</span>
                                                </div>
                                                <div class="divrow actions" :class="isEdit ? 'isDisabled' : ''">
                                                    <a @click="EditChacterModal(element, $t('button.edit'), 'edit')
                                                    ...
                                                    <a @click="removeInfoModal(element)"
                                                        :class="isEdit ? 'pointer-events-none' : ''">
                    ...
                    <div class="credit-crew-area">
                        <div class="login-wrap">
                            <div class="box" v-click-outside="() => (shows1 = false)">
                                <div class="form-group search-area">
                                    <input type="text" v-model="search_crew_text" class="form-control"
                                        @keyup.enter="getCreditCrewList" :placeholder="$t('button.search')"
                                        @change="resetCrewList" />
                                ...
                                <div class="search-wrap-area search-scrollable" v-show="shows1"
                                    @scroll="onScroll($event, 'crew')">
                                    ...
                                    <ul v-else>
                                        <li :class="isEdit ? 'isDisabled' : ''">
                                            ...
                                            <a :class="isEdit ? 'pointer-events-none' : ''" @click="createCrewInfoModal">{{
                                                $t("credit.create")
                                            }} +</a>
                                        </li>
                                    </ul>
                        ...
                        <div class="credit-list-area">
                            <template v-if="crew_details?.length > 0">
                                <draggable :disabled="isEdit" v-model="crew_details" item-key="id" :animation="300"
                                    :forceFallback="true" :scroll-sensitivity="200">
                                    <template #item="{ element }">
                                        <div class="single-credit-list" v-show="element.action_type != 'd'">
                                            ...
                                            <div class="end-part">
                                                ...
                                                <div class="divrow actions" :class="isEdit ? 'isDisabled' : ''">
                                                    ...
                                                    <a @click="removeCrewInfoModal(element)"
                                                        :class="isEdit ? 'pointer-events-none' : ''">
                                                        <svg width="14" height="14" viewBox="0 0 14 14" fill="none"
                                                            xmlns="http://www.w3.org/2000/svg">
                                                            <path
    ...
    <div class="new-cast-modal">
        <!-- add info modal -->
        <PopUpModal :isModal="isEditChacterModal">
            ...
            <template v-slot:body>
                <div class="cast-details">
                    <div class="cast-left-profile">
                        <img v-if="people.people_poster || people.poster" :src="people.people_poster || people.poster"
                            alt="" class="cast-img" />
                    ...
                    <div class="right">
                        ...
                        <div class="form-grp">
                            ...
                            <textarea class="form-control" v-model="description"
                                :placeholder="$t('episode.typeHere')"></textarea>
                        </div>
                        <div class="form-grp">
            ...
            <template v-slot:footer>
                <div class="button-group">
                    <button class="black-outline-btn" :disabled="character_name == '' || description == null || description == '' || is_guest.toString() == ''
                        "
                        :class="character_name == '' || description == null || description == '' || is_guest.toString() == '' ? 'isDisabled' : ''"
                        @click="editCharacterModal">
                        {{ $t("button.save") }}
                    </button>
        ...
        <PopUpModal :isModal="isAddCharacterModal">
            <template v-slot:header>
                <h1>{{ $t('addNewPeople.mediadetails.addButton') }} {{ $t("credit.information") }}</h1>
            </template>
            <template v-slot:body>
                <div class="cast-details">
                    <div class="img-picker">
                        <input type="file" v-on:change="onFileChange" id="img-picker-1" accept="image/*" />
                        <label for="img-picker-1"><img src="@/assets/images/no-image-view.png" alt="" class="cast-img"
                                v-if="!url" />
            ...
            <template v-slot:footer>
                <div class="button-group">
                    <button class="black-outline-btn" :disabled="character_name == '' || description == null || description == '' || is_guest == '' || url == '' || isCreateNewCast
                        " :class="character_name == '' ||
        description == null || description == '' || is_guest == '' || url == '' || isCreateNewCast ? 'isDisabled' : ''"
                        @click="createCharacterInfo">
                        {{ $t("button.save") }}
                    </button>
                    <button class="red-fill-btn" @click="closeModal">
                        {{ $t("button.cancel") }}
                    ...
                    <div class="right">
                        <div class="form-grp">
                            ...
                            <SearchDropdown @dropdownValue="getJobId" :search_type="job" :searchTitleListItem="jobTitleList"
                                searchtitle="department_name" searchvalue="department_value"
                                :labelItem="$t('credit.selectJobTitle')" :clearable="false" />
                        </div>
                    </div>
                </div>
            </template>
        ...
        <PopUpModal :isModal="isCrewCreateInfoModal">
            ...
            <template v-slot:body>
                <div class="cast-details">
                    <div class="img-picker">
                        <input type="file" v-on:change="onFileChange" id="img-picker-1" accept="image/*" />
                        <label for="img-picker-1"><img v-if="!url" src="@/assets/images/no-image-view.png" alt=""
                                class="cast-img" />
                            <img :src="url" alt="" class="cast-img" v-if="url" /></label>
                    </div>
                    <div class="right">
...
const people_id = ref("");
const isAddCharacterModal = ref(false);
const job = ref(null);
...
const isEditChacterModal = ref(false)
// on language change
watch(
    () => props.site_language,
    function (n, o) {
...
const characterRequestList = () => {
    let creditCastList;
    if (route.params.type == "internal") {
        creditCastList = addWebtoonService.editCreditRequestList(
            title_id,
        ...
        creditCastList = addWebtoonService.creditRequestList(
            draft_request_id.value || routeId.value,
            props.site_language,
            "character",
            season_id.value
    ...
    creditCastList
        .then((res) => {
            if (res.status == 200) {
                castRequestList.value = res.data.character_details;
                ...
                for (let i = 0; i < castRequestList.value.length; i++) {
                    castRequestList.value[i]["exist_id"] = castRequestList.value[i].id;
                    ...
                    cast_details.value.push({
                        ...
                        description: castRequestList.value[i].description,
...
const creditCrewRequestList = () => {
    let creditCrewList;
    if (route.params.type == "internal") {
        creditCrewList = addWebtoonService.editCreditRequestList(
            title_id,
            draft_request_id.value || routeId.value,
            props.site_language,
            "crew",
        ...
        creditCrewList = addWebtoonService.creditRequestList(
            draft_request_id.value || routeId.value,
            props.site_language,
            "crew",
            season_id.value
...
const editCharacterModal = () => {
    ...
    if (castDetailsData) {
        let castDetailsData2 = cast_details.value.find((item) => {
            return (item.character_name == character_name.value);
        });
        if (!castDetailsData2) {
            castDetailsData.description = description.value;
            ...
            if (castDetailsData2 && guestClick.value) {
                ...
                if (castDetailsData.action_type == "a") {
                    ...
                    //     1
                    // );
                    castDetailsData2.description = description.value;
    ...
    isEditChacterModal.value = false;
    character_name.value = "";
...
const saveCreditDetails = () => {
    ...
    if (route.params.type == "internal") {
        ...
        saveCreditDetails = addWebtoonService.editCreditDetails(credentials);
    } else {
        saveCreditDetails = addWebtoonService.addCreditDetails(credentials);
    }
    saveCreditDetails
        .then((res) => {
            if (res.status == 200) {
...
const createCharacterInfo = () => {
    ...
    formData.append("credit_type", "character");
    formData.append("description", description.value);
    formData.append("character_name", character_name.value);
    formData.append("is_guest", is_guest.value);

    let createCharacterCredit;
    if (route.params.type == "internal") {
        // formData.append("title_id", title_id);
        createCharacterCredit = addWebtoonService.editCreateNewCredit(formData, title_id);
    } else {
        formData.append("site_language", props.site_language);
        formData.append("draft_request_id", draft_request_id.value || routeId.value
            ? draft_request_id.value || routeId.value
            : "");
        createCharacterCredit = addWebtoonService.createNewCredit(formData);
    }
    createCharacterCredit
        .then((res) => {
            if (res.status == 200) {
                // console.log(res.data.character_details);
                ...
                if (!castDetailsData2) {
                    cast_details.value.push({
                        ...
                        description: description.value,
                        is_guest: is_guest.value,
                        poster: res.data.character_details.image_location,
                        id: cast_details.value.length + 1,
                        action_type: "a",
                    });
                }

                isAddCharacterModal.value = false;
                character_name.value = "";
                description.value = null;
                is_guest.value = "";
                people_id.value = "";
...
const addNewCharacter = () => {
    isAddCharacterModal.value = true;
    url.value = "";
    const body = document.body;
    body.classList.add("dark-modal-open");
};
...
const EditChacterModal = (item, type, cast) => {
    isEditChacterModal.value = true;
    character_name.value = item.character_name ? item.character_name : "";
    description.value = item.description ? item.description : null;
    is_guest.value = item.is_guest == 1 || item.is_guest == 0 ? item.is_guest : "";
    people_id.value = item.people_id;
    people.value = item;
    addType.value = type;
    castType.value = cast;
