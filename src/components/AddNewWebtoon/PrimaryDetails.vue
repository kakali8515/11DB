      ...
      <div class="login-wrap">
        <div class="box">
          <form @submit.prevent="">
            <div class="divRows">
              ...
              <div class="div3colmns">
                <div class="form-group">
                  <label>{{ $t("addNewWebtoonForm.NAVERID") }}</label>
                  <input type="text" v-model="naver_id" class="form-control" placeholder="" maxlength="255"
                    @keypress="isIntegerNumber($event)" :class="error.naver_id ? 'error-border' : ''"
                    :disabled="isEdit" />
                  <span class="error-txt" v-if="error.naver_id">{{
                    error.naver_id
                  }}</span>
                </div>
                <div class="form-group">
                  <label>{{ $t("addNewWebtoonForm.KAKAOID") }}</label>
                  <input type="text" v-model="kakao_id" class="form-control" placeholder="" maxlength="255"
                    @keypress="isIntegerNumber($event)" :class="error.kakao_id ? 'error-border' : ''"
                    :disabled="isEdit" />
                  <span class="error-txt" v-if="error.kakao_id">{{
                    error.kakao_id
                  }}</span>
                </div>
                <div class="form-group">
              ...
              <div class="div1colmns">
                <div class="form-group">
                  <input v-model="title_name" type="text" class="form-control"
                    :placeholder="$t('primaryDetails.typeWebtoonTitle')" @keyup="error.title_name = ref('')"
            ...
            <div class="divRows">
              <h3>{{ $t("addNewWebtoonForm.rating") }}</h3>
              <div class="div1colmns">
                <div class="form-group">
                  <input v-model="rating" type="text" class="form-control" :placeholder="$t('primaryDetails.typeRating')"
                    @keypress="isNumberDecimal($event)" maxlength="4" :class="error.rating ? 'error-border' : ''"
                    :disabled="isEdit" />
            ...
            <div class="divRows">
              <h3>{{ $t("addNewWebtoonForm.telecastdate") }}</h3>
              <div class="div1colmns">
                <div class="form-group">
                  <ul class="webtoon-checkbox">
                    <li v-for="(item, index) in Object.entries(weekdaysList)" :key="index">
                      <label class="checkbox-input">
                        {{ $t(item[1]) }}
                        <input type="checkbox" :value="item[0]" v-model="weekly_telecast_days"
                          @click="getWeekDay($event, item[0])" :disabled="isEdit" />
            ...
            <div class="divRows mt-0">
              ...
              <div class="div1colmns nogap">
                <SearchVideo @searchInput="getConnectionSearch" :list="connectionSearchList"
                  ...
                  title_type="webtoon" :isEdit="isEdit" @scrolling="onConnectionScroll" :page="page" />
              </div>
            </div>
          </form>
        </div>
...
const { t } = i18n.global;
import { requiredField, isIntegerNumber, isValidUrl, isNumberDecimal } from "@/mixins/CommonValidation.js";
...
const search_type = ref("webtoons");
...
const weekly_telecast_days = ref([]);
const page = ref(1)
const limit = ref(10);
const totalRecords = ref(0);
const edit_weekly_telecast_days = ref([]);
...
const getWeekdaysList = () => {
  weekdaysList.value = [];
  addWebtoonService.getWebtoonWeekdays()
    .then((res) => {
      if (res.status == 200) {
        weekdaysList.value = res.data.weekdays_list;
      }
    })
    .catch((err) => {
      return;
...
const isSaveBtn = () => {
  ...
  if (naver_id.value) {
    if (isNaN(naver_id.value)) {
      error.value.naver_id = computed(() => t("Error.onlyNumber"));
    } else {
      error.value.naver_id = ref("");
    }
  }

  if (kakao_id.value) {
    if (isNaN(kakao_id.value)) {
      error.value.kakao_id = computed(() => t("Error.onlyNumber"));
    } else {
      error.value.kakao_id = ref("");
    }
  }
  if (tmdb_id.value) {
  ...
  if (rating.value != "" && (rating.value < 0.1 || rating.value > 10)) {
    error.value.rating = computed(() => t("Error.webrating"));
  }
...
const getWeekDay = (event, day) => {
  if (route.params.type == "internal") {
    if (event.target.checked == true) {
      edit_weekly_telecast_days.value.push({
        day: day,
        id: "",
      });
    } else {
      let data = edit_weekly_telecast_days.value.find((f) => {
        return f.day == day;
      });
      edit_weekly_telecast_days.value.splice(edit_weekly_telecast_days.value.indexOf(data), 1);
    }
  } else {
    if (event.target.checked == true) {
      weekly_telecast_days.value.push(day)
    } else {
      const index = weekly_telecast_days.value.indexOf(day);
      weekly_telecast_days.value.splice(index, 1)
    }
  }
};
...
const savePrimaryDetails = () => {
  const draft_ids = JSON.parse(localStorage.getItem("draft_ids") || "[]");
  originalInputData.value = originalInputData.value.filter((item) => {
  ...
  let credentails = {
    ...
    weekly_telecast_days: route.params.type == "internal" ? edit_weekly_telecast_days.value : weekly_telecast_days.value
  };
  ...
  let saveWebtoonPrimaryDetails;

  if (route.params.type == "internal") {
    saveWebtoonPrimaryDetails = addWebtoonService.editWebtoonPrimaryDetails(credentails);
  } else {
    saveWebtoonPrimaryDetails = addWebtoonService.addWebtoonPrimaryDetails(credentails);
  }

  saveWebtoonPrimaryDetails
    .then((res) => {
      if (res.status == 200) {
        ...
        if (res?.response?.data?.validation?.body?.keys == "naver_id") {
          Toast.fire({ title: t("Error.naverValid") });
          isOnceClickSaveBtn.value = false;
          isSubmit.value = false;
        }
        if (res?.response?.data?.validation?.body?.keys == "kakao_id") {
          Toast.fire({ title: t("Error.kakaoValid") });
          isOnceClickSaveBtn.value = false;
          isSubmit.value = false;
        }
        isOnceClickSaveBtn.value = false;
...
const getPrimaryDetails = () => {
  ...
  let getWebtoonPrimaryDetails;

  if (route.params.type == "internal") {
    getWebtoonPrimaryDetails = addWebtoonService.getEditWebtoonPrimaryDetails(
      title_id.value,
      draft_request_id.value,
      props.site_language,
    );
  } else {
    if (draft_request_id.value) {
      getWebtoonPrimaryDetails = addWebtoonService.getPrimaryDetails(
        draft_request_id.value,
        props.site_language,
      );
    }
  ...
  if (draft_request_id.value || title_id.value) {
    getWebtoonPrimaryDetails.then((res) => {
      if (res.status == 200) {
        emit("selectedId", {
        ...
        naver_id.value = res.data.naver_id || naver_id.value;
        ...
        kakao_id.value = res.data.kakao_id || kakao_id.value;
        ...
        if (route.params.type == "internal") {
          Array.from(res.data.getWeeklyTelecast_list).forEach((item) => {
            edit_weekly_telecast_days.value.push({
              day: item.day,
              id: item.id,
            });
            weekly_telecast_days.value.push(item.day);
          });
        } else {
          weekly_telecast_days.value = res.data.weekly_telecast_details;
        }
      }
    })
      .catch((err) => {
