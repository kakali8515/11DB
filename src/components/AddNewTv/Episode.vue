  ...
  <div class="primary-details-from">
    <div
      ...
      <div class="media-tmtb-outer">
        ...
        <div class="media-save-btn">
          <a class="saveBtn" @click="isEdit == true ? isEditBtn() : (isSubmit = true)">{{ isEdit ? $t("button.edit") :
            $t("button.save") }}</a>
        </div>
      </div>
    </div>
    ...
    <div class="episode-list">
      <ul class="list-wrapper" v-if="episodeList.length > 0" :class="isEdit ? 'isDisabled' : ''">
        <li class="episode-card" v-for="(item, index) in episodeList" :key="index"
          :class="isEdit ? 'pointer-events-none' : ''">
          <div class="close-cross" :class="isEdit ? 'pointer-events-none' : ''"
            @click="closeEpisodeTab(item.episode_number)">
          ...
          <div class="right-content" @click="getSavedEpisodeDeatils(item)">
            <div class="card-img-wrap">
              <img v-if="item.episode_image" :src="item.episode_image" alt="" class="card-img" />
            ...
            <div class="episode-details">
              <span class="episode-no">{{ $t("episode.episode") }}
                {{
                  item.episode_number > 9
                  ? item.episode_number
                  : "0" + item.episode_number
                }}</span>
              <h1 class="episode-title">{{ item.episode_title }}</h1>
              <h3 class="release-date" v-if="item.episode_date">
                {{ dateformat(item.episode_date, 'MMMM DD, YYYY') }}
              </h3>
              <h3 class="release-date" v-if="item.release_date">
                {{ dateformat(item.release_date, 'MMMM DD, YYYY') }}
              </h3>
          ...
          <div class="episode-modal-content">
              ...
              <div class="upload-options">
                <label>
                  <span class="camera-upload">
                    ...
                    <input type="file" class="image-upload" id="image_file" @change="getImage($event)" accept="image/*"
                      style="opacity: 0" />
                  </span>
                </label>
              </div>
            ...
            <div class="episode-row">
              <div class="divRows">
                ...
                <div class="div1colmns">
                  <div class="form-group">
                    ...
                    <span class="error-txt" v-if="errorData.episode_number">{{
              ...
              <div class="divRows">
                ...
                <div class="div1colmns">
                  <div class="form-group">
                    <input type="text" class="form-control" :placeholder="$t('episode.selectEpisodeTitle')"
                      v-model="episode_title" maxlength="150" :class="errorData.episode_title ? 'error-border' : ''" />
                    <span class="error-txt" v-if="errorData.episode_title">{{ errorData.episode_title }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="divRows">
                <h3>{{ $t("episode.overview") }}</h3>
                <div class="div1colmns">
                  <div class="form-group">
                    <textarea v-model="episode_plot" class="form-control" :placeholder="$t('episode.typeHere')"
                      :class="errorData.episode_plot ? 'error-border' : ''"></textarea>
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  props: ["site_language", "requiredId", "saveIdList"],
  emits: ["selectedId", "isEditOn"],
  mixins: [isIntegerNumber, isCharWithSpeailChar],
  ...
  setup(props, { emit }) {
    ...
    const tmdbRefreshClick = () => {
      tmdbEpisodeList.value = episodeList.value;
      tmdbEpisodeList.value.map((i) => {
        i.action_type = "d";
      });
      episodeList.value = [];
      allEpisodeList.value = [];
    ...
    const allEpisodes = async (draft_season_id, exist_season_id) => {
      ...
      try {
        ...
        if (route.params.type == "internal") {
          if (istmDBIdClick.value) {
            let data = {
              ...
              search_text: search_title.value,
              title_id: title_id.value,
              season_id: Number(exist_season_id),
              tmdb_id: tmdb_id.value,
              season_no: selectedSeason.value.season_no
            };
            response = await editTVService.tmdbEpisodeList(data);
          } else {
            response = await editTVService.searchEditEpisodes(data);
          }
        } else {
          response = await addNewTVService.searchEpisodes(data);
        }
        ...
        if (routeId.value) {
          ...
          tmdb_id.value = response.data.tmdb_id ? response.data.tmdb_id : tmdb_id.value;
        }
        allEpisodeList.value = response.data.results;
        allEpisodeList.value.map((i) => {
          i.action_type = istmDBIdClick.value ? "a" : "e";
        });
    ...
    const saveEpisodeList = async () => {
      ...
      let data = {
        draft_request_id: routeId.value, // required
        draft_season_id: draft_season_id.value, // required
        draft_episode_id: draft_episode_id.value, // not for the first time
        site_language: props.site_language, // required
        episode_details: [],
      };
      ...
      // Object.assign(test, { action_type: "d" });
      if (closeEpi.value == true) {
        ...
        // allEpisodeList.value.filter((i) => !episodeList.value.some((j) => i.episode_number === j.episode_number)).forEach((item) => {
        //   Object.assign(item, { action_type: istmDBIdClick.value ? "a" : "e" });
        //   episodeList.value.push(item);
        // });
      }

      //console.log(episodeList.value)
      let episodemergedArray = [...episodeList.value, ...tmdbEpisodeList.value];
      //eturn
      if (episodeList.value.length > 0) {
        if (route.params.type == "internal") {
          Array.from(episodemergedArray).forEach((item) => {
            // if (item.action_type) {
            data.episode_details.push({
              action_type: item.action_type,
              name: item.episode_title,
              description: item.episode_summary,
      ...
      try {
        let res;
        if (route.params.type == "internal") {
          res = await editTVService.saveEditEpisode(data);
        } else {
          res = await addNewTVService.saveEpisode(data);
        }
        if (res.status == 200) {
    ...
    const getSavedEpisodeDeatils = async (item) => {
      if (item.action_type == "a") {
        ...
        try {
          ...
          if (route.params.type == "internal") {
            if (istmDBIdClick.value) {
              res = await editTVService.getTmdbEpisodeDetails(
                item.id,
                item.episode_number,
                title_id.value,
                item.season_id,
                selectedSeason.value.season_no,
                tmdb_id.value,
                props.site_language,
              );
            } else {
              res = await editTVService.getEditEpisodeDetails(
                props.site_language,
                routeId.value,
                item.id,
                item.episode_number,
            ...
            res = await addNewTVService.getEpisodeDetails(
              props.site_language,
              routeId.value,
              item.draft_episode_id,
              item.episode_number,
              item.tmdb_id,
              item.season_no ? item.season_no : ""
            );
          }
    ...
    const addNewEpisodePrev = async () => {
      ...
      formData.append(
        "date",
        episode_realese_date.value ? episode_realese_date.value : ""
      );
      formData.append("overview", episode_plot.value ? episode_plot.value : "");
        ...
        if (
          ...
          try {
            ...
            if (res.status == 200) {
              ...
              // console.log('checking', episodeList.value,data, editedSeasonNo.value, episode_number.value);
              let editedEpisodeIndex = episodeList.value.findIndex(
                // (i) => i.episode_number == data.episode_number
              ...
              // searchForEpisodes(draft_season_id.value);
              // allEpisodes(draft_season_id.value);
            } else {
              errorData.value.episode_number = ref("");
              errorData.value.episode_title = ref("");
              Toast.fire({ title: res.response.data.error });
    ...
    //  input limit
    const isMaxmimum = (evt) => {
      let val = String.fromCharCode(evt.keyCode);
      if (val <= 9 && val >= 1) {
        return true;
      } else {
        evt.preventDefault();
    ...
    return {
      ...
      episode_title,
      episode_plot,
      episode_realese_date,
      isSubmit,
      getSavedEpisodeDeatils,
      errorData,
      draft_season_id,
      openEpisodeModal,
      isIntegerNumber,
